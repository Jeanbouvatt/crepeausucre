...
#setting up security
CONF_FILE="cloud_native_app/frontend/www/conf"
if [ "$1" == "reset-security" ]
then
...
yes | ./setup_service.sh "s" $SUB_SECU "$DIR/s" $KEY_NAME $KEY_FILE "i/install.sh" $NETWORK_NAME $CONF_FILE &
yes | ./setup_service.sh "b" $SUB_SECU "$DIR/b" $KEY_NAME $KEY_FILE "i/install.sh" $NETWORK_NAME $CONF_FILE &
yes | ./setup_service.sh "w" $SUB_SECU "$DIR/w" $KEY_NAME $KEY_FILE "i/install.sh" $NETWORK_NAME $CONF_FILE &
yes | ./setup_service.sh "p" $SUB_SECU "$DIR/p" $KEY_NAME $KEY_FILE "i/install.sh" $NETWORK_NAME $CONF_FILE & 
